...
/**
 * Turns a stroke-order SVG into a [StrokeDiagram].
 *
 * Two sources feed this: KanjiVG (the Japanese sets, and kana) and the generator
 * behind the Chinese sets, which converts Make Me a Hanzi's stroke medians into
 * the same shape. Both are machine generated and rigid, so a couple of anchored
 * regular expressions are enough -- and unlike an XML pull parser they run in
 * plain JVM unit tests.
 *
 * The strokes live before the stroke-number group and are already in stroke
 * order. The marker is "StrokeNumbers" without KanjiVG's namespace prefix, so
 * either source is accepted.
 */
object StrokeSvgParser {
    ...
    private const val NUMBERS_MARKER = "StrokeNumbers"
