...
class StrokeSvgParserTest {

    @Test
    fun readsViewBoxStrokesAndStrokeNumbers() {
        val diagram = StrokeSvgParser.parse(SAMPLE)
    ...
    fun theIdAttributeIsNotMistakenForPathData() {
        ...
        val first = StrokeSvgParser.parse(SAMPLE).strokes.first()
    ...
    fun strokeOrderFollowsDocumentOrder() {
        val strokes = StrokeSvgParser.parse(SAMPLE).strokes
    ...
    fun aDiagramWithoutStrokeNumbersStillParses() {
        ...
        val diagram = StrokeSvgParser.parse(svg)
    ...
    fun aDiagramWithNoStrokesIsEmptyButValid() {
        val diagram = StrokeSvgParser.parse("<svg viewBox=\"0 0 109 109\"></svg>")
